...
This is the orchestration layer the CLI and GUI both call into. It
doesn't implement any signing or device-protocol logic itself -- it just
sequences the pieces in signing/, device/, and apple/ in the right order
...
from .ipa_archive import cleanup, extract_ipa, repack_ipa
...
class SideloadOptions:
    ...
    signing_backend: Optional[BackendName] = None  # None = auto-pick
    install_to_device: bool = True
    device_udid: Optional[str] = None
    keep_signed_ipa: Optional[Path] = None  # if set, copy the final signed IPA here
...
async def run_sideload(
    ...
    """
    Runs the full pipeline and returns the path to the final signed .ipa.
    Raises SideloaderError (or a subclass) on any failure, with the
    progress callback having already reported how far it got.
    """
